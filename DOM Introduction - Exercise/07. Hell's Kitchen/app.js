function solve() {
   ...
   function onClick() {
      ...
      result.forEach(w => print += `Name: ${w[0]} With Salary: ${Number(w[1])} `);
      document.querySelector('#bestRestaurant p').textContent = `Name: ${bestName} Average Salary: ${Number(currentAvgSalary).toFixed(2)} Best Salary: ${Number(result[0][1]).toFixed(2)}`;
