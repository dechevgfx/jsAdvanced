...
describe("Payment Package Tests", () => {
    it("should throw error if name is not a string", () => {
        ...
        expect(() => {
            new PaymentPackage(true, 1500);
        }).to.throw("Name must be a non-empty string");
    });

    it("should throw error if value is NaN or negative", () => {
        expect(() => {
            new PaymentPackage("Engineering", "3");
        }).to.throw("Value must be a non-negative number");
        expect(() => {
            new PaymentPackage("Engineering", "dfsgh");
        }).to.throw("Value must be a non-negative number");
        expect(() => {
            new PaymentPackage("Engineering", {});
        }).to.throw("Value must be a non-negative number");
        expect(() => {
            new PaymentPackage("Engineering", []);
        }).to.throw("Value must be a non-negative number");
        expect(() => {
            new PaymentPackage("Engineering", true);
        }).to.throw("Value must be a non-negative number");
    });

    it("is has two valid parameters", () => {
        const obj = new PaymentPackage("Engineering", 1700);
        expect(obj.name === "Engineering").to.be.true;
        expect(obj.value === 1700).to.be.true;
    });

    it("should change the value property when tried to be set", () => {
        const obj = new PaymentPackage("Engineering", 1500);
        obj.value = 1900;
        expect(obj.value).to.equal(1900);
    });

    it("should change the name property when tried to be set", () => {
        const obj = new PaymentPackage("Engineering", 1500);
        obj.name = "Marketing";
        expect(obj.name).to.equal("Marketing");
    });

    it("should get the VAT property when instantiated", () => {
        const obj = new PaymentPackage("Engineering", 1500);
        expect(obj.VAT).to.equal(20);
    });

    it("should set the VAT property", () => {
        const obj = new PaymentPackage("Engineering", 1500);
        obj.VAT = 50;
        expect(obj.VAT).to.equal(50);
    });

    it("should throw error when given VAT parameter is NaN or negative", () => {
        const obj = new PaymentPackage("Engineering", 1500);
        expect(() => {
            obj.VAT = {};
        }).to.throw("VAT must be a positive number");
        expect(() => {
            obj.VAT = "30";
        }).to.throw("VAT must be a positive number");
        expect(() => {
